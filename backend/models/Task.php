...
class Task {
    ...
    // NUEVO MÉTODO PARA EL CRUD COMPLETO
    public static function update(int $id, string $titulo, string $descripcion, string $estado, string $prioridad, ?string $fechaLimite, ?int $responsableId): bool {
        $db = getDB();
        $stmt = $db->prepare("
            UPDATE tasks 
            SET titulo = ?, descripcion = ?, estado = ?, prioridad = ?, fecha_limite = ?, responsable_id = ? 
            WHERE id = ?
        ");
        $stmt->execute([$titulo, $descripcion, $estado, $prioridad, $fechaLimite, $responsableId, $id]);
        return $stmt->rowCount() > 0;
    }
}